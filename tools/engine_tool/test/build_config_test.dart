...
int main() {
  test('BuildConfig parser works', () {
    final BuildConfig buildConfig = BuildConfig.fromJson(
      'linux_test_config',
      convert.jsonDecode(buildConfigJson) as Map<String, Object?>,
    );
    expect(buildConfig.valid, isTrue);
    expect(buildConfig.error, isNull);
...
''';
    final BuildConfig buildConfig = BuildConfig.fromJson(
      'linux_test_config',
      convert.jsonDecode(invalidInput) as Map<String, Object?>,
    );
    expect(buildConfig.valid, isFalse);
    expect(buildConfig.error, equals('"builds" field must be a list.\n'));
  });
...
''';
    ...
    expect(buildConfig.builds[0].error, equals(
...
''';
    ...
    expect(buildConfig.builds[0].ninja.error, equals(
...
''';
    ...
    expect(buildConfig.builds[0].tests[0].error, equals(
      '"language" field must be a string.\n',
    ));
  });
...
''';
    ...
    expect(buildConfig.builds[0].generators[0].error, equals(
      '"name" field must be a string.\n',
    ));
  });
...
''';
    ...
    expect(buildConfig.builds[0].archives[0].error, equals(
      '"name" field must be a string.\n',
    ));
  });
...
''';
    ...
    expect(buildConfig.tests[0].error, equals(
      '"name" field must be a string.\n',
    ));
  });
...
''';
    ...
    expect(buildConfig.tests[0].tasks[0].error, equals(
      '"name" field must be a string.\n',
    ));
  });
  return 0;
